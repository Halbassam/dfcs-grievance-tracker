...
-- Complete database setup (Supabase Postgres)
--
-- Run this ONE file in the Supabase SQL Editor to set up
-- EVERYTHING needed for a brand-new install:
--   1. All application tables (grievances, activity, archive,
--      users with admin/steward roles, sessions, holidays,
--      dropdown lists, email log)
--   2. The full default dropdown lists already filled in —
--      Status, Bureau, Location, County, Bargaining Unit, Job
--      Classification, Shift, every CBA Article, every Grievance
--      Type, every Activity Type, and a starting Steward roster
--   3. The 2024–2027 holiday calendar
--
-- After running this, the only manual step left is creating your
-- first login account from inside the app itself (Settings >
-- Manage users) — see the README for the full walkthrough. That
-- first account is automatically made an Admin.
--
-- SAFE TO RE-RUN: every statement uses IF NOT EXISTS or ON CONFLICT
-- DO NOTHING, so running this file again never duplicates data or
-- overwrites anything you've already customized from inside the
-- app's Settings tab.
--
...
-- means a frontend field that doesn't have its own column never
-- gets silently dropped on save — it round-trips through the JSONB
-- blob exactly the way the original JSON-file version kept it.
-- ================================================================
