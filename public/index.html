...
let ORG_SETTINGS = { agency: '', localNo: '', bureau: '', location: '', county: '', shift: '' };
...
async function refresh(isBackgroundPoll){
  try{
    ...
    ORG_SETTINGS = data.orgSettings || { agency: '', localNo: '', bureau: '', location: '', county: '', shift: '' };
    updatePageTitle();
...
function clearForm(){
  ['f-gid','f-name','f-grievant-email','f-section','f-remedy'].forEach(id=>$(id).value='');
...
function renderSettingsCounts(isBackgroundPoll){
  ...
  const orgFields = ['orgAgencyInput','orgLocalNoInput','orgBureauInput','orgLocationInput','orgCountyInput','orgShiftInput'];
...
$('orgSettingsSaveBtn').addEventListener('click', async ()=>{
  ...
  btn.disabled = true; btn.textContent = 'Saving…';
  try{
    await apiPost('/api/org-settings', { agency, localNo, bureau, location, county, shift });
    ORG_SETTINGS = { agency, localNo, bureau, location, county, shift };
    updatePageTitle();
